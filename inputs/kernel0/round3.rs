...
struct list_head<'a> {
    next: Option<&'a mut list_head<'a>>,
    prev: Option<&'a mut list_head<'a>>,
}
...
fn main() {
    ...
    let atomic = atomic_t { counter: 0 };

    let m = mutex {
        owner: atomic_long_t { counter: 0 },
        wait_lock: raw_spinlock_t {
    ...
    perf_event_max_stack_handler(&mut table, w, &mut buf, &mut len, &mut pos, atomic, m);
    *table.extra1 = 1;
}
